...
enumerate::enumerate(string * n, set< string > *s):basictype(n, 3)
{
  enumvalue = s;
  if (s != NULL) {
    values = vector<string>(s->begin(), s->end());
  }
...
int
enumerate::get_value_count()
{
  return values.size();
}

pair<string, BDD>
enumerate::get_value(Cudd * bddmgr, vector<BDD> * v, int value_index)
{
  ...
  string value_name = values[value_index];
  vector<bool> *vindex = get_value_index(value_name);
