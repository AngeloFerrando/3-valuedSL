...
rangedint::build_all_BDDs(Cudd * bddmgr, vector<BDD> * v)
{
  ...
  for (int i = lowerbound; i <= upperbound; i++) {
    ...
    BDD value_bdd = bddmgr->bddOne();
    for (unsigned int j = 0; j < vindex->size(); j++) {
      if ((*vindex)[j] == 1) {
        value_bdd *= (*v)[get_index_begin() + j];
      } else {
        value_bdd *= ~(*v)[get_index_begin() + j];
      }
    }
...
rangedint::build_individual_BDD(Cudd * bddmgr, vector<BDD> *v, vector< BDD* > *bddblocks, unsigned int count) {
	int index_begin = get_index_begin();
	unsigned int length = upperbound - lowerbound;
	if(length > count) {
...
int
rangedint::get_value_count()
{
  return get_upperbound() - get_lowerbound() + 1;
}

pair<string, BDD>
rangedint::get_value(Cudd * bddmgr, vector<BDD> * v, int value_index)
{
  if (value_index < 0 || value_index >= get_value_count()) {
    cout << "Invalid value index '" << value_index << "'" << endl;
    exit(1);
  }

  int value = get_lowerbound() + value_index;

  stringstream value_name_stream;
  value_name_stream << value;
  string value_name = value_name_stream.str();

  vector<int> *vindex = get_value_index(value);
  ...
  return make_pair(value_name, value_bdd);
}
