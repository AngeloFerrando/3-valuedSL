...
global_consistency_check()
{
  ...
  /*
   * Check perfect recall formulae
   */
  if (options["verbosity"] > 4) {
    cout << endl << "Checking perfect recall formula definition ..." << endl;
  }
  count = 0;
  for (vector<modal_formula>::iterator i = is_perfect_recall_formulae->begin();
       i != is_perfect_recall_formulae->end(); i++) {
    if (options["verbosity"] > 4) {
      cout << "  formula " << ++count << " ... " << endl;
    }
    if (!i->check_perfect_recall_atomic_proposition()) {
      is_consistent = false;
    }
  }
