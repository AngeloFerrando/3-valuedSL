...
%token AGENT ACTIONS PROTOCOL EVOLUTION END LB RB LCB RCB LSB RSB LT GT IF EQ AND
...
%token FORMULAE IMPLIES AG EG AX EX AF EF A E UNTIL K GK GCK O DK
...
%type <boolean_flag> perfectrecalltag
...
| ID gplprefix gplsufffix {
  if($1!=NULL && $2==1 && $3==1) {
    (*is_groups)[*$1] = set<string>();
    free($1);
  }
 }
| groupdeflist ID gplprefix gplsufffix {
  if($2!=NULL && $3==1 && $4==1) {
    if(is_groups->find(*$2) != is_groups->end() && options["quiet"] == 0)
      cout << "warning: group " << *$2 << " already exists." << endl;
    (*is_groups)[*$2] = set<string>();
    free($2);
  }
 }
| error {
...
formlist: perfectrecalltag formula SEMICOLON {
  if($2!=NULL) {
    ($1 ? is_perfect_recall_formulae : is_formulae)->push_back(*$2);
  }
 }
| perfectrecalltag formula error {
  driver.syntax_error = true;
  USE($1);
  USE($2);
 }
| formlist perfectrecalltag formula SEMICOLON {
  if($3!=NULL) {
    ($2 ? is_perfect_recall_formulae : is_formulae)->push_back(*$3);
  }
 }
| formlist perfectrecalltag formula error {
  driver.syntax_error = true;
  USE($2);
  USE($3);
 }
...
| LT LT ID GT GT formula %prec QUANTIFIER {
  // SL <<x>> operator
  if ($3 != NULL && $6 != NULL) {
    modal_formula *var = new modal_formula(new atomic_proposition($3));
    $$ = new modal_formula(50, var, $6);
  } else {
    if ($3 != NULL) {
      free($3);
    }
...
| LSB LSB ID RSB RSB formula %prec QUANTIFIER {
  // SL [[x]] operator
  if ($3 != NULL && $6 != NULL) {
    modal_formula *var = new modal_formula(new atomic_proposition($3));
    $$ = new modal_formula(51, var, $6);
  } else {
    if ($3 != NULL) {
      free($3);
    }
...
| LB ID COMMA ID RB formula %prec QUANTIFIER {
  // SL (a, x) operator
  if ($2 != NULL && $4 != NULL && $6 != NULL) {
    modal_formula *agent = new modal_formula(new atomic_proposition($2));
    modal_formula *var = new modal_formula(new atomic_proposition($4));
    $$ = new modal_formula(52, agent, var, $6);
  } else {
    if ($2 != NULL) {
      free($2);
    }
...
| LB ENVIRONMENT COMMA ID RB formula %prec QUANTIFIER {
  // SL (a, x) operator
  if ($4 != NULL && $6 != NULL) {
