...
public:
  ...
  virtual pair<string, BDD> get_value(Cudd * bddmgr, vector<BDD> * v,
                                      int value_index);
};
...
public:
  ...
  int get_value_count();
  pair<string, BDD> get_value(Cudd * bddmgr, vector<BDD> * v, int value_index);
};
...
class modal_formula:public Object
{
  /* 0: term, 1=AND, 2=OR, 3=NOT, 4=IMPLIES,
     ...
     50=<<x>> (SL existential quantifier),
     51=[[x]] (SL universal quantifier),
     52=(a, x) (SL agent binding),
     53=X (LTL/SL operator),
     54=F (LTL/SL operator),
     55=G (LTL/SL operator),
     56=U (LTL/SL operator)
   */
  unsigned char op;
  Object *obj[3];
  bool check_atomic_proposition(unsigned char type, set<string> *variables,
      set<string> *agents);
  bool check_sl_quantifier_syntax(set<string> *variables, set<string> *agents);
  bool check_sl_binding_syntax(set<string> *variables, set<string> *agents);
  bool check_sl_temporal_syntax(set<string> *agents);
  void get_sharing_agents(string variable_name,
      set<basic_agent *> *sharing_agents);
  bool check_sl_1g_prefix();
  BDD check_formula(bdd_parameters * para, map<string, strategy *> *variables,
      map<string, strategy *> *agents);
  pair<BDD, strategy> check_sl_quantified_subformula(bdd_parameters * para,
      map<string, strategy *> *variables, map<string, strategy *> *agents);
  BDD check_sl_binding_subformula(bdd_parameters * para,
      map<string, strategy *> *variables, map<string, strategy *> *agents);
...
public:
  ...
  modal_formula(modal_formula * f);
  ~modal_formula();
  unsigned char get_op();
  Object *get_operand(unsigned char i);
  string to_string() const;
  bool check_atomic_proposition();
  bool check_perfect_recall_atomic_proposition();
  BDD check_formula(bdd_parameters * para);
  set<basic_agent *> get_sharing_agents(string variable_name);
...
BDD pick_combination_minterm(BDD bdd, map<string, strategy *> *variables,
    bdd_parameters *para);
BDD pick_next_combination_minterm(BDD bdd, map<string, strategy *> *variables,
    bdd_parameters *para);
...
public:
  ...
  vector<BDD> get_visible_variables(vector<BDD> * v);
...
class strategy_entry
{
  private:
    BDD state;
    BDD variables;
    map<string, BDD> actions;
  public:
    strategy_entry(BDD state, BDD variables, map<string, BDD> actions);
    ~strategy_entry();
    BDD *get_state();
    BDD *get_variables();
    map<string, BDD> *get_actions();
};
...
class strategy
{
  ...
  public:
    strategy(vector<strategy_entry> action_mapping, vector<BDD> bdd_variables,
             set<basic_agent *> sharing_agents, BDD valid_restrictions);
    ~strategy();
    vector<strategy_entry> *get_action_mapping();
    vector<BDD> *get_bdd_variables();
    set<basic_agent *> *get_sharing_agents();
    BDD *get_valid_restrictions();
    void print_strategy(bdd_parameters *para, BDD *restricted_state);
};
