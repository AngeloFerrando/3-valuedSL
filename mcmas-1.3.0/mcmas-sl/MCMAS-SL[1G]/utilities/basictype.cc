...
int
basictype::get_value_count()
{
  return 2;
}

pair<string, BDD>
basictype::get_value(Cudd * bddmgr, vector<BDD> * v, int value_index)
{
  switch (value_index) {
    case 0:
      return make_pair("false", BDD(~(*v)[get_index_begin()]));
    case 1:
      return make_pair("true", BDD((*v)[get_index_begin()]));
    default:
