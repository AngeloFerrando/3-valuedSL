#include <ctime>
#include "types.hh"
#include "sl_imperfect_recall.hh"
...
bool
modal_formula::check_perfect_recall_atomic_proposition()
{
  bool b1, b2;
  ...
  switch (op) {
    ...
    case 56:      // SL U
      return ((modal_formula *) obj[0])
          ->check_perfect_recall_atomic_proposition()
          && ((modal_formula *) obj[1])
          ->check_perfect_recall_atomic_proposition();
    case 3:       // NOT
    case 53:      // SL X
    ...
    case 14:      // AF
    case 15:      // EF
      cout << "    CTL operators are not supported in "
          << "perfect recall semantics." << endl;
      break;
    case 30:      // K
    case 31:      // GK
    case 32:      // GCK
    case 42:      // DK
      cout << "    epistemic operators are not supported in "
          << "perfect recall semantics." << endl;
      break;
    case 40:      // O
      cout << "    deontic operators are not supported in "
          << "perfect recall semantics." << endl;
      break;
    case 45:      // ATL X
    case 46:      // ATL F
    case 47:      // ATL G
    case 48:      // ATL U
      cout << "    ATL operators are not supported in "
          << "perfect recall semantics." << endl;
      break;
  }
  return false;
...
bool
modal_formula::check_sl_1g_prefix()
{
  set<string> variables;
  set<string> agents;

  modal_formula *formula = this;
  ...
  while (!break_loop) {
    switch(formula->op) {
      ...
      case 51:
        {
          string variable_name = ((atomic_proposition *)
              ((modal_formula *) formula->obj[0])->obj[0])->get_proposition();
          variables.insert(variable_name);
          formula = ((modal_formula *) formula->obj[1]);
          break;
        }
      case 52:
        {
          string agent_name = ((atomic_proposition *)
              ((modal_formula *) formula->obj[0])->obj[0])->get_proposition();
          string variable_name = ((atomic_proposition *)
              ((modal_formula *) formula->obj[1])->obj[0])->get_proposition();
          ...
          if (variables.find(variable_name) == variables.end()) {
            ...
            map<string, basic_agent*>::iterator agent_it = is_agents->find(
                agent_name);
            map<string, set<string> >::iterator group_it = is_groups->find(
                agent_name);

            if (agent_it != is_agents->end()) {
              agents.insert(agent_name);
            } else if (group_it != is_groups->end()) {
              agents.insert(group_it->second.begin(), group_it->second.end());
            } else {
              cout << "    agent " << agent_name << " is not defined." << endl;
              return false;
            }
            formula = ((modal_formula *) formula->obj[2]);
          }
          break;
        }
      default:
  ...
  for (map<string, basic_agent*>::iterator it = is_agents->begin();
       it != is_agents->end(); it++) {
    if (agents.find(it->first) == agents.end()) {
      cout << "    agent " << it->first << " is not bound in the prefix."
          << endl;
      return false;
    }
  }
  return formula->check_perfect_recall_atomic_proposition();
}
  ...
  case 55:      // SLG
    {
      ...
      // Generate all transitions and states.
      int loopstart_index = tmpresult.size() - 1;
