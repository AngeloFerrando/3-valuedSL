               ...
               bdd_parameters *para)
{
  ...
  if (options["verbosity"] > 5) {
    cout << "BDD variables before: " << next_bdd_variable << endl;
  }
  ...
  if (options["verbosity"] > 5) {
    cout << "BDD variables after: " << next_bdd_variable << endl;
  }
                        ...
                        vector<BDD> *bdd_variables, bdd_parameters *para)
{
  ...
  long long possible_strategies = 1;
  ...
  for (list<pair<BDD, set<string> > >::iterator i =
    ...
    pair<BDD, map<string, BDD> > entry = convert_actions_to_bdds(&i->second,
        bdd_variables, para);
    possible_strategies *= entry.second.size();
    action_mapping.push_back(strategy_entry(i->first, entry.first,
        entry.second));
  }

  if (options["verbosity"] > 5) {
    cout << "Possible strategies: " << possible_strategies << endl;
  }
